...
export function decodeFirestoreDocument(doc: FirestoreDocument | null | undefined): Record<string, any> {
  if (!doc?.fields) {
    return {};
  }
  const result: Record<string, any> = {};
...
export async function listFirestoreCollection(
  ...
  const pageSize = options.pageSize ? String(options.pageSize) : "200";

  do {
    const url = new URL(`${FIRESTORE_BASE_URL}/documents/${collectionPath}`);
    ...
    const data = await res.json().catch(() => ({}));
    if (!res.ok) {
      const message = data?.error?.message || "Nie udało się pobrać danych z Firestore.";
      throw Object.assign(new Error(message), { status: res.status });
    }
    const batch: FirestoreDocument[] = Array.isArray(data?.documents) ? data.documents : [];
...
export async function createFirestoreDocument(
  ...
  if (!res.ok) {
    const data = await res.json().catch(() => ({}));
    const message = data?.error?.message || "Nie udało się zapisać dokumentu Firestore.";
    throw Object.assign(new Error(message), { status: res.status });
  }
}
...
export async function patchFirestoreDocument(
  ...
  const url = new URL(`${FIRESTORE_BASE_URL}/documents/${documentPath}`);
  ...
  if (!res.ok) {
    const data = await res.json().catch(() => ({}));
    const message = data?.error?.message || "Nie udało się zaktualizować dokumentu Firestore.";
    throw Object.assign(new Error(message), { status: res.status });
  }
}
